...
/**
 * Scans for classes annotated with @Conductor and registers them.
 */
public class ConductorScanner {
    private static final Logger logger = LoggerFactory.getLogger(ConductorScanner.class);
    
    /**
     * Scans the specified package for Conductor classes.
     */
    public List<ConductorMethod> scan(String basePackage) {
        ...
        try {
            ...
            for (Class<?> clazz : classes) {
                if (clazz.isAnnotationPresent(Conductor.class)) {
                    ...
                    methods.addAll(processConductorClass(clazz));
                }
            }
            
            logger.info("Found {} conductor methods", methods.size());
            return methods;
